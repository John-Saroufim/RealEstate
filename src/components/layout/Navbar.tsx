...
export function Navbar() {
  ...
  const isApp = ["/dashboard", "/workout-tracker", "/progress", "/ai-coach", "/settings"].some(
    (p) => location.pathname.startsWith(p)
  );

  const links = isApp ? [...appLinks, ...navLinks.slice(0, 3)] : navLinks;
            ...
            <div className="px-4 py-4 space-y-1">
              ...
              <div className="pt-3 border-t border-border flex gap-2">
                <Link to="/dashboard" className="flex-1" onClick={() => setMobileOpen(false)}>
                  <Button className="w-full gradient-neon-bg text-primary-foreground font-semibold">
                    Get Started
                  </Button>
                </Link>
              </div>
